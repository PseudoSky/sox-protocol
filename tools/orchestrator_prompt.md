...
| `Resolve architect questions` | **Run proactive scan + architect dispatches without advancing any phases.** Read `.workflow/plans/bucket-classification/result.md`'s queue and `classified.json#/architect_questions`. For each orphan question (no corresponding ADR phase in any engagement), dispatch `workflow:workflow-architect`. Record decisions at `docs/decisions/<slug>.md`. Update `.workflow/decisions/INDEX.md`. Then exit without entering the main loop. See `ORCHESTRATOR-CONTRACT.md §Architect-question resolution flow`. |
...
**Architect-question resolution runs proactively at the start of every invocation** (except `Resolve architect questions` which runs ONLY that step). After the seven-step startup checklist, before entering the main loop, scan `.workflow/plans/bucket-classification/result.md` for the question queue, classify each as `delegated` (an engagement's ADR phase will resolve it) or `orphan` (no ADR maps to it), dispatch `workflow:workflow-architect` for orphans, record decisions at `docs/decisions/<slug>.md`. Do NOT skip this step on subsequent runs — re-scan every time so newly-added questions get picked up. See `ORCHESTRATOR-CONTRACT.md §Architect-question resolution flow`.

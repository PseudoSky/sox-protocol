...
> `{{send_tool}}`, `{{recv_tool}}`, `{{subscribe_tool}}`, and `{{list_tool}}`
...
Call `{{send_tool}}` with a `channel` name and a `body` object:
...
`{{send_tool}}` returns as soon as the message is durably accepted. Proceed
...
synthesising a final output. Call `{{recv_tool}}` with no arguments to drain
...
2. **Send.** Call `{{send_tool}}` with a `clarification_request` body.
...
5. **Drain at checkpoints.** Call `{{recv_tool}}` before each major decision.
...
When `{{recv_tool}}` returns a `clarification_reply` that contradicts your
working assumption:
...
**Send-and-wait** — calling `{{send_tool}}` and then making no progress until
